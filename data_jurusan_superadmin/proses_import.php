...
if (isset($_POST['btn-import'])) {
    ...
    foreach ($data as $indeks => $row) {
        ...
        if ($rv == 0) {
            // simpan ke database
            $query_simpan = mysqli_query($db, "INSERT INTO tbl_jurusan VALUES ('$kode_jurusan', '$nama_jurusan')") or die(mysqli_error($db));
        }
    }
    unlink($alamat_tujuan); // hapus file
    echo '<script>alert ("Data jurusan berhasil diimport")</script>';
